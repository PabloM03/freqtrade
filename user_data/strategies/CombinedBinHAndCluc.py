...
class CombinedBinHAndCluc(IStrategy):
    ...
    - Compras por BinHV45, Cluc y RSI sobreventa (original) + rebote confirmado desde mínimos.
    - Ventas con señales técnicas + trailing stop sensible.
    """
    ...
    trailing_stop_positive_offset = 0.04    # se activa a partir de +4%
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # NUEVO: Mínimo de 20 velas (para el rebote)
        dataframe['ll20'] = ta.MIN(dataframe['low'], timeperiod=20)

        return dataframe

    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # Lógica original (3 bloques)
        cond_original = (
            (
                # BinHV45
        ...
        # NUEVO: Rebote confirmado desde mínimo reciente (opcional, suma entradas)
        cond_rebote = (
            (dataframe['close'] < dataframe['ema_slow']) &
            (dataframe['rsi'] < 40) &
            (dataframe['ll20'] > 0) &
            (dataframe['close'] >= dataframe['ll20'] * 1.01) &   # +1% desde el mínimo 20 velas
            (dataframe['rsi'] > dataframe['rsi_prev']) &
            (dataframe['close'] > dataframe['close'].shift(1))
        )

        dataframe.loc[cond_original | cond_rebote, 'buy'] = 1
        return dataframe
    ...
    def custom_exit(
        ...
        **kwargs
        ...
        # No forzar venta por debajo del precio de compra (deja actuar stoploss/trailing)
        ...
        # TP discreto moderado (por si aún no está activo el trailing)
