...
SLIPPAGE_BUFFER = 0.0004        # 🏃 Margen extra para cubrir deslizamiento en la ejecución de órdenes. Rango típico: 0.0002-0.001. Subirlo exige más beneficio antes de vender.
MIN_PROFIT_NET = 3 * FEE_RATE + SLIPPAGE_BUFFER  # 📈 Beneficio neto mínimo requerido para vender, considerando comisiones y deslizamiento. Rango típico: 0.002-0.004. Subirlo exige más beneficio antes de vender.
PEAK_MIN_PROFIT = 0.0075        # 🏔️ Beneficio mínimo para permitir salida en pico óptimo (máximos locales). Rango típico: 0.004-0.01. Subirlo hace más exigente la venta en picos.
HH_EMA_MIN_PROFIT = 0.0095      # 📊 Beneficio mínimo para salida por ruptura de EMA8 tras un máximo. Rango típico: 0.006-0.012. Subirlo hace más difícil vender tras máximos.
HARD_TP = 0.025                 # 🎯 Take profit fijo para asegurar ganancias si se alcanza. Rango típico: 0.01-0.03. Subirlo busca ganancias mayores pero puede perder retrocesos.

# --- Stoploss y trailing ---
STOPLOSS_ABS = -0.058           # 🛑 Stoploss absoluto para limitar pérdidas máximas por operación. Rango típico: -0.03 a -0.08. Subirlo (menos negativo) reduce pérdidas pero puede saltar antes.
TRAIL_ATR_MULT_LOW = 1.9        # 🐢 Multiplicador de ATR para trailing stop si beneficio bajo (stop más ajustado). Rango típico: 1.5-2.5. Subirlo aleja el trailing stop.
TRAIL_ATR_MULT_HIGH = 2.6       # 🦅 Multiplicador de ATR para trailing stop si beneficio alto (stop más holgado). Rango típico: 2.0-3.0. Subirlo aleja el trailing stop en beneficios altos.
TRAIL_DIST_MIN = 0.015          # 📏 Distancia mínima para trailing stop, evita stops demasiado ajustados. Rango típico: 0.01-0.02. Subirlo da más margen antes de saltar el stop.
TRAIL_DIST_MAX = 0.045          # 📏 Distancia máxima para trailing stop, evita stops demasiado lejanos. Rango típico: 0.025-0.04. Subirlo permite stops más lejanos.
...
ADX_STRONG_TREND = 24           # 💪 Valor mínimo de ADX para considerar tendencia fuerte (mayor protección trailing). Rango típico: 20-35. Subirlo exige tendencia más fuerte para trailing holgado.
...
# --- Anti-cuchillo ---
PCT1_MIN = -1.2                 # 🔪 Caída máxima en 1 vela para permitir compra (evita comprar en caídas bruscas). Rango típico: -1.0 a -2.0. Bajarlo permite compras en caídas más fuertes.
PCT3_MIN = -3.5                 # 🔪 Caída máxima en 3 velas para permitir compra (protege de tendencias bajistas fuertes). Rango típico: -2.0 a -4.0. Bajarlo permite compras en tendencias más bajistas.
...
NO_BUY_EMA20_MULT = 1.003       # 🚫 Multiplicador de EMA20 para evitar compras "arriba". Rango típico: 1.0-1.05. Subirlo permite comprar más alto.
NO_BUY_RSI_MIN = 62             # 🚫 RSI mínimo para evitar compras en sobrecompra. Rango típico: 55-65. Subirlo evita compras en zonas más sobrecompradas.

# --- Zonas de valor para comprar ---
DEEP_BB = 0.22                  # 🏦 Profundidad máxima de BB% para considerar compra en zona muy baja. Rango típico: 0.15-0.25. Subirlo permite compras menos profundas.
BB_ZONE_OK = 0.40               # 🏦 BB% máximo para considerar zona de compra aceptable. Rango típico: 0.3-0.45. Subirlo permite compras en zonas menos bajas.
...
# A) Mínimo local
A_LL10_MULT = 1.0055            # 📉 Multiplicador para comparar el mínimo local con el mínimo de las últimas 10 velas. Rango típico: 1.002-1.01. Subirlo exige mínimos más bajos para detectar valle.
A_RSI_PREV_MAX = 48             # 📉 RSI máximo previo para permitir compra en giro alcista tras sobreventa. Rango típico: 40-50. Subirlo permite compras con menos sobreventa previa.
...
D_PCT3_MAX = -3.2               # 💥 Caída máxima en 3 velas para detectar capitulación. Rango típico: -3.0 a -5.0. Bajarlo detecta caídas más fuertes.
...
D_TAIL_ATR_MULT = 0.95          # 💥 Multiplicador de ATR para la cola de la vela (mecha larga indica rebote). Rango típico: 0.8-1.5. Subirlo exige mechas más largas.
# E) Pullback a EMA8
...
E_BB_MID_MULT = 1.015           # 🔄 Multiplicador para comparar el precio con la banda media BB. Rango típico: 1.005-1.02. Subirlo exige precios más bajos respecto a la banda media.
...
F_LL10_LOWER = 0.985            # 🏞️ Multiplicador inferior para doble toque. Rango típico: 0.98-0.995. Bajarlo permite más diferencia entre toques.

# --- Ventas ---
REJECT_UPPER_ATR_MULT = 0.90    # 🚩 Multiplicador de ATR para detectar mecha superior grande. Rango típico: 0.8-1.2. Subirlo exige mechas más largas para vender.
REJECT_WICK_BODY_RATIO = 1.10   # 🚩 Relación mecha/cuerpo para identificar rechazo fuerte. Rango típico: 1.1-1.4. Subirlo exige mechas más largas respecto al cuerpo.
SELL_RSI_PEAK = 67              # 🚩 RSI mínimo para vender en pico. Rango típico: 65-75. Subirlo exige sobrecompra más fuerte.
...
CRASH_ATR_BREAK_MULT = 1.4      # ⚡ Multiplicador de ATR para detectar ruptura fuerte bajo EMA. Rango típico: 1.3-2.0. Subirlo exige rupturas más grandes.
CRASH_ADX_MIN = 20              # ⚡ ADX mínimo para considerar crash. Rango típico: 18-28. Subirlo exige tendencia bajista más fuerte.
CRASH_RSI_MAX = 55              # ⚡ RSI máximo para crash. Rango típico: 45-52. Subirlo permite crash-guard con menos sobreventa.
...
STARTUP_CANDLES = 110           # ⏰ Número de velas iniciales requeridas para calcular indicadores. Rango típico: 50-150. Subirlo mejora precisión de indicadores largos.

# --- Bollinger config ---
BB40_WINDOW = 40                # 📊 Ventana de velas para Bollinger Bands largas. Rango típico: 30-60. Subirlo suaviza las bandas.
BB40_STDS = 2.0                 # 📊 Desviaciones estándar para BB40. Rango típico: 1.8-2.5. Subirlo amplía las bandas.
BB20_WINDOW = 20                # 📊 Ventana de velas para Bollinger Bands cortas. Rango típico: 15-30. Subirlo suaviza las bandas.
BB20_STDS = 2.0                 # 📊 Desviaciones estándar para BB20. Rango típico: 1.8-2.5. Subirlo amplía las bandas.
...
MAX_PCT_UP_3 = 3.5              # % máx. subida en 3 velas para permitir compra
MAX_GREEN_STREAK = 4            # nº máx. de velas verdes recientes; si hay racha >= N, no comprar
BUY_BELOW_EMA20_MULT = 1.000    # exigir que el precio esté por DEBAJO de EMA20 (0.998 = -0.2%)
BUY_BELOW_BB_MID_MULT = 1.000   # exigir que el precio esté por DEBAJO de la banda media BB
BB_EXPANDING_HIGH = 0.65        # si bb_percent >= 0.55 y bb_expanding, no comprar (expansión arriba)
PUMP_VOL_MULT = 2.0             # volumen de la vela > 1.7x media rápida => posible pump (bloquear)
NEAR_HH_DISTANCE = 0.0045       # no comprar si el precio está a <0.3% del máximo 20 velas
