...
SLIPPAGE_BUFFER = 0.0008        # 🏃 Margen extra para cubrir deslizamiento en la ejecución de órdenes. Rango típico: 0.0002-0.001. Subirlo exige más beneficio antes de vender.
MIN_PROFIT_NET = 8 * FEE_RATE + SLIPPAGE_BUFFER  # 📈 Beneficio neto mínimo requerido para vender, considerando comisiones y deslizamiento. Rango típico: 0.002-0.004. Subirlo exige más beneficio antes de vender.
...
HH_EMA_MIN_PROFIT = 0.015       # 📊 Beneficio mínimo para salida por ruptura de EMA8 tras un máximo. Rango típico: 0.006-0.012. Subirlo hace más difícil vender tras máximos.
HARD_TP = 0.040                 # 🎯 Take profit fijo para asegurar ganancias si se alcanza. Rango típico: 0.01-0.03. Subirlo busca ganancias mayores pero puede perder retrocesos.
...
TRAIL_ATR_MULT_HIGH = 3.6       # 🦅 Multiplicador de ATR para trailing stop si beneficio alto (stop más holgado). Rango típico: 2.0-3.0. Subirlo aleja el trailing stop en beneficios altos.
...
TRAIL_DIST_MAX = 0.065          # 📏 Distancia máxima para trailing stop, evita stops demasiado lejanos. Rango típico: 0.025-0.04. Subirlo permite stops más lejanos.
TRAIL_VERTICAL_MIN = 0.030      # 🚀 Distancia mínima para trailing si hay rally vertical. Rango típico: 0.015-0.03. Subirlo da más margen en subidas rápidas.
ADX_STRONG_TREND = 29           # 💪 Valor mínimo de ADX para considerar tendencia fuerte (mayor protección trailing). Rango típico: 20-35. Subirlo exige tendencia más fuerte para trailing holgado.
ROC5_VERTICAL = 3.8             # 📈 ROC5 mínimo para considerar rally vertical. Rango típico: 2-5. Subirlo exige movimientos más bruscos para activar trailing vertical.
...
PCT3_MIN = -7.0                 # 🔪 Caída máxima en 3 velas para permitir compra (protege de tendencias bajistas fuertes). Rango típico: -2.0 a -4.0. Bajarlo permite compras en tendencias más bajistas.
...
BB_ZONE_OK = 0.36               # 🏦 BB% máximo para considerar zona de compra aceptable. Rango típico: 0.3-0.45. Subirlo permite compras en zonas menos bajas.
...
# A) Mínimo local
A_LL10_MULT = 1.0060            # 📉 Multiplicador para comparar el mínimo local con el mínimo de las últimas 10 velas. Rango típico: 1.002-1.01. Subirlo exige mínimos más bajos para detectar valle.
...
D_PCT3_MAX = -4.0               # 💥 Caída máxima en 3 velas para detectar capitulación. Rango típico: -3.0 a -5.0. Bajarlo detecta caídas más fuertes.
...
# E) Pullback a EMA8
E_RSI_MIN = 47                  # 🔄 RSI mínimo para permitir pullback alcista. Rango típico: 40-50. Subirlo exige más fuerza en el rebote.
...
E_BB_MID_MULT = 0.999           # 🔄 Multiplicador para comparar el precio con la banda media BB. Rango típico: 1.005-1.02. Subirlo exige precios más bajos respecto a la banda media.
...
MAX_PCT_UP_1 = 0.50             # % máx. subida en 1 vela para permitir compra (usa misma escala que PCT1_MIN: en %)
MAX_PCT_UP_3 = 1.50             # % máx. subida en 3 velas para permitir compra
MAX_GREEN_STREAK = 2            # nº máx. de velas verdes recientes; si hay racha >= N, no comprar
BUY_BELOW_EMA20_MULT = 0.992    # exigir que el precio esté por DEBAJO de EMA20 (0.998 = -0.2%)
BUY_BELOW_BB_MID_MULT = 0.992   # exigir que el precio esté por DEBAJO de la banda media BB
BB_EXPANDING_HIGH = 0.48        # si bb_percent >= 0.55 y bb_expanding, no comprar (expansión arriba)
PUMP_VOL_MULT = 2.0             # volumen de la vela > 1.7x media rápida => posible pump (bloquear)
NEAR_HH_DISTANCE = 0.0200       # no comprar si el precio está a <0.3% del máximo 20 velas
