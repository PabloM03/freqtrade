...
SLIPPAGE_BUFFER = 0.0009        # 🏃 Margen extra para cubrir deslizamiento en la ejecución de órdenes. Rango típico: 0.0002-0.001. Subirlo exige más beneficio antes de vender.
MIN_PROFIT_NET = 7 * FEE_RATE + SLIPPAGE_BUFFER  # 📈 Beneficio neto mínimo requerido para vender, considerando comisiones y deslizamiento. Rango típico: 0.002-0.004. Subirlo exige más beneficio antes de vender.
PEAK_MIN_PROFIT = 0.012         # 🏔️ Beneficio mínimo para permitir salida en pico óptimo (máximos locales). Rango típico: 0.004-0.01. Subirlo hace más exigente la venta en picos.
HH_EMA_MIN_PROFIT = 0.014       # 📊 Beneficio mínimo para salida por ruptura de EMA8 tras un máximo. Rango típico: 0.006-0.012. Subirlo hace más difícil vender tras máximos.
...
# --- Stoploss y trailing ---
STOPLOSS_ABS = -0.045           # 🛑 Stoploss absoluto para limitar pérdidas máximas por operación. Rango típico: -0.03 a -0.08. Subirlo (menos negativo) reduce pérdidas pero puede saltar antes.
TRAIL_ATR_MULT_LOW = 2.3        # 🐢 Multiplicador de ATR para trailing stop si beneficio bajo (stop más ajustado). Rango típico: 1.5-2.5. Subirlo aleja el trailing stop.
TRAIL_ATR_MULT_HIGH = 3.2       # 🦅 Multiplicador de ATR para trailing stop si beneficio alto (stop más holgado). Rango típico: 2.0-3.0. Subirlo aleja el trailing stop en beneficios altos.
...
TRAIL_DIST_MAX = 0.055          # 📏 Distancia máxima para trailing stop, evita stops demasiado lejanos. Rango típico: 0.025-0.04. Subirlo permite stops más lejanos.
TRAIL_VERTICAL_MIN = 0.026      # 🚀 Distancia mínima para trailing si hay rally vertical. Rango típico: 0.015-0.03. Subirlo da más margen en subidas rápidas.
...
ROC5_VERTICAL = 3.2             # 📈 ROC5 mínimo para considerar rally vertical. Rango típico: 2-5. Subirlo exige movimientos más bruscos para activar trailing vertical.
FALLBACK_TRAIL_DIST = 0.022     # 🛟 Distancia fallback si falla el cálculo de trailing dinámico. Rango típico: 0.012-0.025. Subirlo da más margen de seguridad.

# --- Anti-cuchillo ---
PCT1_MIN = -0.7                 # 🔪 Caída máxima en 1 vela para permitir compra (evita comprar en caídas bruscas). Rango típico: -1.0 a -2.0. Bajarlo permite compras en caídas más fuertes.
PCT3_MIN = -1.8                 # 🔪 Caída máxima en 3 velas para permitir compra (protege de tendencias bajistas fuertes). Rango típico: -2.0 a -4.0. Bajarlo permite compras en tendencias más bajistas.
COOLDOWN_BARS = 5               # 🧊 Número de velas de enfriamiento tras una vela roja grande. Rango típico: 2-6. Subirlo aumenta el tiempo sin comprar tras caídas fuertes.

# --- Filtro de compras altas ---
NO_BUY_BB_MULT = 1.004          # 🚫 Multiplicador de la banda media BB para evitar compras "arriba". Rango típico: 1.01-1.15. Subirlo permite comprar más alto.
...
DEEP_BB = 0.18                  # 🏦 Profundidad máxima de BB% para considerar compra en zona muy baja. Rango típico: 0.15-0.25. Subirlo permite compras menos profundas.
...
LOWER_WICK_BODY_RATIO = 1.25    # 🕯️ Relación mecha inferior/cuerpo para identificar velas tipo martillo. Rango típico: 1.1-1.3. Subirlo exige mechas más largas para considerar giro.

# --- Reglas de compra específicas ---
# A) Mínimo local
A_LL10_MULT = 1.0070            # 📉 Multiplicador para comparar el mínimo local con el mínimo de las últimas 10 velas. Rango típico: 1.002-1.01. Subirlo exige mínimos más bajos para detectar valle.
...
C_STOCH_MAX = 30                # 📉 Valor máximo de StochRSI para considerar sobreventa y posible rebote. Rango típico: 30-40. Subirlo permite compras con menos sobreventa.
# D) Capitulación
D_PCT1_MAX = -2.0               # 💥 Caída máxima en 1 vela para detectar capitulación. Rango típico: -1.5 a -2.5. Bajarlo detecta capitulaciones más bruscas.
D_PCT3_MAX = -3.8               # 💥 Caída máxima en 3 velas para detectar capitulación. Rango típico: -3.0 a -5.0. Bajarlo detecta caídas más fuertes.
...
D_TAIL_ATR_MULT = 1.20          # 💥 Multiplicador de ATR para la cola de la vela (mecha larga indica rebote). Rango típico: 0.8-1.5. Subirlo exige mechas más largas.
# E) Pullback a EMA8
E_RSI_MIN = 48                  # 🔄 RSI mínimo para permitir pullback alcista. Rango típico: 40-50. Subirlo exige más fuerza en el rebote.
E_LL10_MULT = 1.012             # 🔄 Multiplicador para comparar el mínimo con el mínimo de 10 velas. Rango típico: 1.005-1.02. Subirlo exige mínimos más bajos.
E_BB_MID_MULT = 1.008           # 🔄 Multiplicador para comparar el precio con la banda media BB. Rango típico: 1.005-1.02. Subirlo exige precios más bajos respecto a la banda media.
...
F_LL10_LOWER = 0.990            # 🏞️ Multiplicador inferior para doble toque. Rango típico: 0.98-0.995. Bajarlo permite más diferencia entre toques.
...
REJECT_WICK_BODY_RATIO = 1.20   # 🚩 Relación mecha/cuerpo para identificar rechazo fuerte. Rango típico: 1.1-1.4. Subirlo exige mechas más largas respecto al cuerpo.
SELL_RSI_PEAK = 70              # 🚩 RSI mínimo para vender en pico. Rango típico: 65-75. Subirlo exige sobrecompra más fuerte.
SELL_RSI_REJECT = 62            # 🚩 RSI mínimo para vender por rechazo en zona alta. Rango típico: 55-65. Subirlo exige más sobrecompra para vender por rechazo.
SELL_RSI_HH_EMA = 62            # 🚩 RSI mínimo para vender tras ruptura de EMA8 en máximos. Rango típico: 58-65. Subirlo exige más sobrecompra.
SELL_RSI_WICK = 62              # 🚩 RSI mínimo para vender por mecha superior grande. Rango típico: 58-65. Subirlo exige más sobrecompra.

# --- Crash-guard ---
CRASH_FAST_DROP_EMA8 = 0.989    # ⚡ Multiplicador para detectar caída rápida bajo EMA8. Rango típico: 0.99-0.995. Bajarlo detecta caídas más leves.
CRASH_FAST_DROP_PCT1 = -0.7     # ⚡ Caída máxima en 1 vela para crash-guard. Rango típico: -0.5 a -1.0. Bajarlo detecta caídas más leves.
CRASH_ATR_BREAK_MULT = 1.5      # ⚡ Multiplicador de ATR para detectar ruptura fuerte bajo EMA. Rango típico: 1.3-2.0. Subirlo exige rupturas más grandes.
CRASH_ADX_MIN = 22              # ⚡ ADX mínimo para considerar crash. Rango típico: 18-28. Subirlo exige tendencia bajista más fuerte.
CRASH_RSI_MAX = 52              # ⚡ RSI máximo para crash. Rango típico: 45-52. Subirlo permite crash-guard con menos sobreventa.
...
MAX_PCT_UP_1 = 0.9              # % máx. subida en 1 vela para permitir compra (usa misma escala que PCT1_MIN: en %)
MAX_PCT_UP_3 = 2.5              # % máx. subida en 3 velas para permitir compra
...
BUY_BELOW_BB_MID_MULT = 0.998   # exigir que el precio esté por DEBAJO de la banda media BB
BB_EXPANDING_HIGH = 0.55        # si bb_percent >= 0.55 y bb_expanding, no comprar (expansión arriba)
PUMP_VOL_MULT = 1.8             # volumen de la vela > 1.7x media rápida => posible pump (bloquear)
NEAR_HH_DISTANCE = 0.0080       # no comprar si el precio está a <0.3% del máximo 20 velas
