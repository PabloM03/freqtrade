...
PEAK_MIN_PROFIT = 0.003         # 🏔️ Beneficio mínimo para permitir salida en pico óptimo (máximos locales). Rango típico: 0.004-0.01. Subirlo hace más exigente la venta en picos.
HH_EMA_MIN_PROFIT = 0.0065      # 📊 Beneficio mínimo para salida por ruptura de EMA8 tras un máximo. Rango típico: 0.006-0.012. Subirlo hace más difícil vender tras máximos.
HARD_TP = 0.045                 # 🎯 Take profit fijo para asegurar ganancias si se alcanza. Rango típico: 0.01-0.03. Subirlo busca ganancias mayores pero puede perder retrocesos.
...
TRAIL_ATR_MULT_LOW = 1.7        # 🐢 Multiplicador de ATR para trailing stop si beneficio bajo (stop más ajustado). Rango típico: 1.5-2.5. Subirlo aleja el trailing stop.
TRAIL_ATR_MULT_HIGH = 2.5       # 🦅 Multiplicador de ATR para trailing stop si beneficio alto (stop más holgado). Rango típico: 2.0-3.0. Subirlo aleja el trailing stop en beneficios altos.
TRAIL_DIST_MIN = 0.014          # 📏 Distancia mínima para trailing stop, evita stops demasiado ajustados. Rango típico: 0.01-0.02. Subirlo da más margen antes de saltar el stop.
TRAIL_DIST_MAX = 0.034          # 📏 Distancia máxima para trailing stop, evita stops demasiado lejanos. Rango típico: 0.025-0.04. Subirlo permite stops más lejanos.
TRAIL_VERTICAL_MIN = 0.019      # 🚀 Distancia mínima para trailing si hay rally vertical. Rango típico: 0.015-0.03. Subirlo da más margen en subidas rápidas.
ADX_STRONG_TREND = 22           # 💪 Valor mínimo de ADX para considerar tendencia fuerte (mayor protección trailing). Rango típico: 20-35. Subirlo exige tendencia más fuerte para trailing holgado.
ROC5_VERTICAL = 2.3             # 📈 ROC5 mínimo para considerar rally vertical. Rango típico: 2-5. Subirlo exige movimientos más bruscos para activar trailing vertical.
FALLBACK_TRAIL_DIST = 0.017     # 🛟 Distancia fallback si falla el cálculo de trailing dinámico. Rango típico: 0.012-0.025. Subirlo da más margen de seguridad.
...
NO_BUY_BB_MULT = 1.03           # 🚫 Multiplicador de la banda media BB para evitar compras "arriba". Rango típico: 1.01-1.15. Subirlo permite comprar más alto.
NO_BUY_EMA20_MULT = 1.005       # 🚫 Multiplicador de EMA20 para evitar compras "arriba". Rango típico: 1.0-1.05. Subirlo permite comprar más alto.
NO_BUY_RSI_MIN = 67             # 🚫 RSI mínimo para evitar compras en sobrecompra. Rango típico: 55-65. Subirlo evita compras en zonas más sobrecompradas.

# --- Zonas de valor para comprar ---
DEEP_BB = 0.24                  # 🏦 Profundidad máxima de BB% para considerar compra en zona muy baja. Rango típico: 0.15-0.25. Subirlo permite compras menos profundas.
BB_ZONE_OK = 0.42               # 🏦 BB% máximo para considerar zona de compra aceptable. Rango típico: 0.3-0.45. Subirlo permite compras en zonas menos bajas.
LOWER_WICK_BODY_RATIO = 1.05    # 🕯️ Relación mecha inferior/cuerpo para identificar velas tipo martillo. Rango típico: 1.1-1.3. Subirlo exige mechas más largas para considerar giro.

# --- Reglas de compra específicas ---
# A) Mínimo local
A_LL10_MULT = 1.0025            # 📉 Multiplicador para comparar el mínimo local con el mínimo de las últimas 10 velas. Rango típico: 1.002-1.01. Subirlo exige mínimos más bajos para detectar valle.
A_RSI_PREV_MAX = 49             # 📉 RSI máximo previo para permitir compra en giro alcista tras sobreventa. Rango típico: 40-50. Subirlo permite compras con menos sobreventa previa.
# B) Re-entrada tras BB baja -> usa BB_ZONE_OK
# C) StochRSI en sobreventa
C_STOCH_MAX = 39                # 📉 Valor máximo de StochRSI para considerar sobreventa y posible rebote. Rango típico: 30-40. Subirlo permite compras con menos sobreventa.
# D) Capitulación
D_PCT1_MAX = -1.4               # 💥 Caída máxima en 1 vela para detectar capitulación. Rango típico: -1.5 a -2.5. Bajarlo detecta capitulaciones más bruscas.
D_PCT3_MAX = -2.8               # 💥 Caída máxima en 3 velas para detectar capitulación. Rango típico: -3.0 a -5.0. Bajarlo detecta caídas más fuertes.
D_BB_PERCENT_MAX = 0.08         # 💥 BB% máximo para capitulación (muy cerca de la banda inferior). Rango típico: 0.03-0.08. Subirlo permite capitulación menos extrema.
D_TAIL_ATR_MULT = 0.9           # 💥 Multiplicador de ATR para la cola de la vela (mecha larga indica rebote). Rango típico: 0.8-1.5. Subirlo exige mechas más largas.
# E) Pullback a EMA8
...
F_BB_PERCENT_MAX = 0.38         # 🏞️ BB% máximo para doble toque en valle. Rango típico: 0.25-0.35. Subirlo permite doble toque en zonas menos bajas.
F_LL10_UPPER = 1.009            # 🏞️ Multiplicador superior para doble toque. Rango típico: 1.002-1.01. Subirlo permite más diferencia entre toques.
F_LL10_LOWER = 0.988            # 🏞️ Multiplicador inferior para doble toque. Rango típico: 0.98-0.995. Bajarlo permite más diferencia entre toques.

# --- Ventas ---
REJECT_UPPER_ATR_MULT = 0.78    # 🚩 Multiplicador de ATR para detectar mecha superior grande. Rango típico: 0.8-1.2. Subirlo exige mechas más largas para vender.
REJECT_WICK_BODY_RATIO = 1.08   # 🚩 Relación mecha/cuerpo para identificar rechazo fuerte. Rango típico: 1.1-1.4. Subirlo exige mechas más largas respecto al cuerpo.
SELL_RSI_PEAK = 61              # 🚩 RSI mínimo para vender en pico. Rango típico: 65-75. Subirlo exige sobrecompra más fuerte.
SELL_RSI_REJECT = 48            # 🚩 RSI mínimo para vender por rechazo en zona alta. Rango típico: 55-65. Subirlo exige más sobrecompra para vender por rechazo.
SELL_RSI_HH_EMA = 54            # 🚩 RSI mínimo para vender tras ruptura de EMA8 en máximos. Rango típico: 58-65. Subirlo exige más sobrecompra.
SELL_RSI_WICK = 54              # 🚩 RSI mínimo para vender por mecha superior grande. Rango típico: 58-65. Subirlo exige más sobrecompra.
...
CRASH_ATR_BREAK_MULT = 1.7      # ⚡ Multiplicador de ATR para detectar ruptura fuerte bajo EMA. Rango típico: 1.3-2.0. Subirlo exige rupturas más grandes.
CRASH_ADX_MIN = 24              # ⚡ ADX mínimo para considerar crash. Rango típico: 18-28. Subirlo exige tendencia bajista más fuerte.
CRASH_RSI_MAX = 46              # ⚡ RSI máximo para crash. Rango típico: 45-52. Subirlo permite crash-guard con menos sobreventa.
...
BB40_STDS = 1.7                 # 📊 Desviaciones estándar para BB40. Rango típico: 1.8-2.5. Subirlo amplía las bandas.
BB20_WINDOW = 20                # 📊 Ventana de velas para Bollinger Bands cortas. Rango típico: 15-30. Subirlo suaviza las bandas.
BB20_STDS = 1.9                 # 📊 Desviaciones estándar para BB20. Rango típico: 1.8-2.5. Subirlo amplía las bandas.

# --- Anti-chase (evitar compras en subidas/picos) ---
MAX_PCT_UP_1 = 1.8              # % máx. subida en 1 vela para permitir compra (usa misma escala que PCT1_MIN: en %)
MAX_PCT_UP_3 = 4.5              # % máx. subida en 3 velas para permitir compra
MAX_GREEN_STREAK = 5            # nº máx. de velas verdes recientes; si hay racha >= N, no comprar
BUY_BELOW_EMA20_MULT = 1.006    # exigir que el precio esté por DEBAJO de EMA20 (0.998 = -0.2%)
BUY_BELOW_BB_MID_MULT = 1.003   # exigir que el precio esté por DEBAJO de la banda media BB
BB_EXPANDING_HIGH = 0.70        # si bb_percent >= 0.55 y bb_expanding, no comprar (expansión arriba)
PUMP_VOL_MULT = 2.0             # volumen de la vela > 1.7x media rápida => posible pump (bloquear)
NEAR_HH_DISTANCE = 0.0040       # no comprar si el precio está a <0.3% del máximo 20 velas
