...
# Crontab (servidor): 0 7 * * 1 → lunes 9:00 AM hora España (UTC+2 verano)
set -euo pipefail
...
L.append(f"📊 Reporte Semanal — lunes {today.strftime('%d %b %Y')}")
...
if [ "$OK" != "True" ] && [ "$OK" != "true" ]; then
    echo "[weekly_report] ERROR Telegram: $RESP" >&2
else
