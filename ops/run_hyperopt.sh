...
# timeout 1800s (30 min) — Kraken requiere --dl-trades (trades individuales, más lento)
# En descargas incrementales (datos existentes) suele terminar en <5min
echo ""
echo "[1/4] Descargando datos 15m desde 20220101 (timeout 30min)..."
timeout 1800 "$FT" download-data $CONF \
    --timeframes 15m \
    --timerange "20220101-${TODAY}" \
    --dl-trades \
    2>&1 | grep -E 'Downloading|Done|pairs|Error|trades' | tail -10 || true
